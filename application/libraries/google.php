...
class google{
	...
	function __construct()
	{
		...
		if(isset($google_config[0]) && $this->CI->session->userdata("user_type") != "Admin")
		{			
		...
		if($this->google_api_key=="")
		{
			...
			shuffle($google_config);
			$randIndex = array_rand($google_config, 1);
			$this->google_api_key=$google_config[$randIndex]["google_safety_api"];
...
//			}
	...
	private function _getRandomApiKey() {
		$where=array("where"=>array("user_type"=>"Admin"));
		$join=array('users'=>"users.id=config.user_id,left");
		$google_config=$this->CI->basic->get_data("config",$where,"",$join);
		shuffle($google_config);
...
//		if (!$content) {
//			$this->_getRandomApiKey();
//			$this->google_page_speed_insight($domain, $strategy);
//		}
...
//			if (!$content) {
//				$this->_getRandomApiKey();
//				$this->get_general_content($url, $proxy);
//			}
...
//			$start = stripos($url, '=') + 1;
//			$this->_getRandomApiKey();
//			$url = substr_replace($url, $this->google_api_key, $start, strlen($this->google_api_key));
//			$this->youtube_video_curl($url);
//		}
