...
class Payment extends Home
{
    ...
    {     
        ...
        if($this->is_demo == '1')$data["xvalue"]["stripe_secret_key"]=$data["xvalue"]["stripe_publishable_key"]="XXXXXXXXXX";
    ...
    public function transaction_log_manual_data() 
    {
        ...
        foreach ($info as $key => $value) {
            ...
            if (! isset($info[$i]['actions'])) {
                ...
                if ('Admin' == $this->session->userdata('user_type')) {
                    $approve_btn = '<a href="#" id="mp-approve-btn" data-toggle="tooltip" title="'.$this->lang->line("Approve").'" class="btn btn-circle btn-outline-success ' . $is_disabled . '" data-id="' . $info[$i]['id'] . '"><i class="fas fa-check-circle"></i></a>';
                    $reject_btn = '<a href="#" id="mp-reject-btn" data-toggle="tooltip" title="'.$this->lang->line("Reject").'" class="btn btn-circle btn-outline-danger ' . $is_disabled . '" data-id="' . $info[$i]['id'] . '"><i class="fas fa-times-circle"></i></a>';
                    ...
                    $output .= '<div class="dropdown-menu mini_dropdown text-center width_120px">';
                    $output .= $approve_btn;
                    $output .= $reject_btn;
                    $output .= '</div>';
                    $output .= '</div>';
                    if($i==0) $output .= '<script src="'.base_url().'assets/js/system/tooltip_popover.js"></script>';
    ...
    private function manual_payment_display_attachment($file) 
    {
        ...
        $output .= '</div>';
        $output .= '</div>';
        $output .= '<script src="'.base_url().'assets/js/system/maual_payment_attachment.js"></script>';
