		...
		$(document).ready(function() {
			...
			$(document).on('click', '#search_submit', function(event) {
				...
				if ($(".daterange-cus_1").val() != "") {
					...
					let redirect_url = '<?php echo base_url('social_accounts/channel_analytics/' . $channel_list_table_id. '/'); ?>' + date_value[0].replaceAll('/', '-').trim() + '/' + date_value[1].replaceAll('/', '-').trim();
			 ...
			 */
			let video_added_vs_removed_info = $("#video_added_vs_removed_info").val();
			video_added_vs_removed_info = JSON.parse(video_added_vs_removed_info);

			let video_added_vs_removed_step_size = (video_added_vs_removed_info.max_val + 2) / 5;

			let videoAddedRemovedChart = document.getElementById("videoAddedRemovedChart").getContext('2d');
			new Chart(videoAddedRemovedChart, {
			  type: 'line',
			  data: {
			    labels: video_added_vs_removed_info.date,
			    datasets: [{
			      label: '<?php echo $this->lang->line("Added"); ?>',
			      data: video_added_vs_removed_info.added,
			      borderWidth: 2,
			      backgroundColor: 'rgba(63,82,227,.8)',
			      borderWidth: 0,
			      borderColor: 'transparent',
			      ...
			      label: '<?php echo $this->lang->line("Removed"); ?>',
			      data: video_added_vs_removed_info.removed,
			      borderWidth: 2,
			      backgroundColor: 'rgba(254,86,83,.7)',
			      borderWidth: 0,
			      borderColor: 'transparent',
			  ...
			  options: {
			    ...
			    scales: {
			      yAxes: [{
			        ...
			        ticks: {
			          beginAtZero: true,
			          stepSize: video_added_vs_removed_step_size
			        }
			      }],
			      xAxes: [{
			        gridLines: {
