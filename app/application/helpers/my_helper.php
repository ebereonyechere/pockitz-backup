...
// thousands to k convert function 
  ...
  function convertDataTableResult($result=array(), $columns=array(), $start=0,$primary_key='id')
    {
        ...
        foreach ($result as $key => $single_row) {
            ...
            if($have_checkbox) 
            {
              ...
              $str ='<input  name="datatableCheckboxRow[]" id="datatableCheckboxRow'.$primary_val.'" class="datatableCheckboxRow regular-checkbox cursor_pointer"  type="checkbox" value="'.$primary_val.'"/> <label for="datatableCheckboxRow'.$primary_val.'"></label>';
              $temp[1] = $str;
            }
...
if ( ! function_exists('addHttpswww'))
{ 
function addHttpswww( $url ){
